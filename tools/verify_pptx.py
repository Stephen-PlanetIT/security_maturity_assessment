...
import os
import sys
# Ensure project root is on sys.path so 'export' can be imported when running from tools/
sys.path.append(os.path.dirname(os.path.abspath(os.path.join(__file__, ".."))))
# Provide a minimal 'streamlit' stub to satisfy config imports when running headless
try:
...
assert any((s.shapes.title and "Agenda" in s.shapes.title.text) for s in p.slides), "Agenda slide missing"
# Maturity Overview is optional unless radar data exists; do not hard-fail here
# assert any((s.shapes.title and "Maturity Overview" in s.shapes.title.text) for s in p.slides), "Maturity Overview slide missing"
...
for s in p.slides:
    try:
        ...
        # Accept either explicit facilitator cue or authoritative static-template notes
        if ("What Good Looks Like" in txt) or ("APPLICATION_DIRECTIVE" in txt) or ("TABLETOP_TEMPLATE_DECK" in txt):
            has_notes = True
            break
    except Exception:
        pass
...
assert any((s.shapes.title and "Priority Actions" in s.shapes.title.text) for s in p.slides), "Priority Actions slide missing"
